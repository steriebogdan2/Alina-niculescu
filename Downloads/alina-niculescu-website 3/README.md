...
## Stare la livrare, 5 august 2026

`npm run build` nu a putut fi rulat: mediul de pregătire nu are acces la
registrul npm, deci dependințele nu s-au putut instala. În schimb, codul a fost
verificat cu compilatorul TypeScript real (versiunea 6), folosind declarații
temporare pentru pachetele lipsă. Rezultatul: **zero erori**.

Verificatorul a fost validat: reintroducerea intenționată a unei proprietăți
inexistente a fost semnalată corect, deci absența erorilor înseamnă ceva.

Ce nu acoperă această verificare: tipurile reale din `@types/react` și
`react-router-dom`, plus etapa de împachetare Vite. Primul lucru de făcut:

    npm install
    npm run build

`npm install` va genera `package-lock.json`. Comite-l în repozitoriu.

## Atenție la publicarea pe GitHub

`public/videos` ocupă 115 MB, adică peste 80% din proiect. Niciun fișier nu
depășește limita de 100 MB per fișier, dar un repozitoriu cu atâta conținut
video se clonează greu. Varianta sănătoasă pe termen lung este mutarea
clipurilor pe un CDN și păstrarea în cod doar a adreselor.
